...
declare_id!("Minr7X8qNpFxS1wXaKjKqLvXqHpFZ9YmN8QhBvK5ZxP");
...
pub mod minerals {
    ...
    pub fn mint_mineral_tokens(ctx: Context<MintMineral>, amount: u64) -> Result<()> {
        ...
        require!(site.owner == ctx.accounts.minter.key(), MineralError::Unauthorized);
        
        let new_extracted = site.extracted_amount
            .checked_add(amount)
            .ok_or(MineralError::OverflowError)?;
            
        require!(new_extracted <= site.estimated_reserve, MineralError::ExceedsReserve);
    ...
    pub fn transfer_mineral(
        ...
        if royalty > 0 {
            ...
            let royalty_ctx = CpiContext::new(ctx.accounts.token_program.to_account_info(), royalty_cpi);
    ...
    pub fn verify_extraction(ctx: Context<VerifyExtraction>, amount: u64) -> Result<()> {
        ...
        require!(site.owner == ctx.accounts.verifier.key(), MineralError::Unauthorized);
        
        site.extracted_amount = site.extracted_amount
            .checked_add(amount)
            .ok_or(MineralError::OverflowError)?;
    ...
    pub fn update_royalty(ctx: Context<UpdateSite>, new_royalty_bps: u16) -> Result<()> {
        ...
        require!(site.owner == ctx.accounts.owner.key(), MineralError::Unauthorized);
