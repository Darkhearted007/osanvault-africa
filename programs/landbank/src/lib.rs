...
declare_id!("Land7X8qNpFxS1wXaKjKqLvXqHpFZ9YmN8QhBvK5ZxP");

#[program]
pub mod landbank {
    use super::*;

    pub fn create_land_pool(ctx: Context<CreatePool>, name: String, min_contribution: u64) -> Result<()> {
        let pool = &mut ctx.accounts.pool;
        
        require!(!name.is_empty(), LandError::InvalidName);
        require!(min_contribution >= 10_000_000, LandError::MinContributionTooLow); // Min $10
    ...
    pub fn contribute(ctx: Context<Contribute>, amount: u64) -> Result<()> {
        ...
        user_position.ownership_bps = user_position.ownership_bps
    ...
    pub fn update_appreciation(ctx: Context<UpdatePool>, new_appreciation_bps: u16) -> Result<()> {
        ...
        msg!("Annual appreciation updated to {} bps", new_appreciation_bps);
        Ok(())
    }
