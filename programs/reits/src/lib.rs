...
pub mod reits {
    ...
    pub fn create_reit(ctx: Context<CreateReit>, name: String, total_shares: u64, nav_per_share: u64) -> Result<()> {
    ...
    pub fn issue_shares(ctx: Context<IssueShares>, recipient: Pubkey, num_shares: u64) -> Result<()> {
        let reit = &mut ctx.accounts.reit;
        
        require!(!reit.paused, ReitError::ReitPaused);
        require!(num_shares > 0, ReitError::InvalidShares);
        
        let new_shares = reit.shares_issued
